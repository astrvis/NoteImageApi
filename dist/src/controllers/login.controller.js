...
import { deleteDbUserSessionSaveNewTen, getDbImageUser, getDbUserByIdFirst, getDbUserSessionFirst, insertDbUserSession, } from "../repositories/imageUser.repo.js";
import { getBrowserOsDevice, getClientIp } from "../utils/comm.js";
export const login = async (c) => {
    const { username, password } = c.req.valid("json");
    const user = await getDbImageUser(username);
    if (!user)
        throw new AppError(404, "用户名不存在");
...
const refreshTokenCookie = async (c, refreshToken, maxAge) => {
...
export const refreshToken = async (c) => {
    ...
    if (!refreshToken)
        throw new AppError(401, "refreshToken不存在");
