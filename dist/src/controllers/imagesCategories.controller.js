...
import { addDbImageCategory, deleteDbImageCategory, getDbAllImageCategory, getDbAllImagesByCategoryId, updateDbImageCategory, } from "../repositories/images.categories.repo.js";
import { imagesCache, imagesCategoriesCache } from "../utils/lruCache.js";
export const addImageCategory = async (c) => {
    ...
    if (!result)
        throw new AppError(409, "图片分类名称已存在");
    return c.json({
        success: true,
        list: {
            id: result.id,
...
export const updateImageCategoryById = async (c) => {
...
export const deleteImageCategoryById = async (c) => {
    const { id } = c.req.valid("param");
...
export const getAllImageCategory = async (c) => {
...
export const getImageCategoryById = async (c) => {
    const { id } = c.req.valid("param");
    const { page, pageSize } = c.req.valid("query");
