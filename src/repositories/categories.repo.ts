...
import { articles, categories, type ArticlesSelect, type CategoriesSelect } from "../db/schema.js"
 ...
 * @returns
 */
export const getDbAllCategories = async (
  page: number,
  pageSize: number,
): Promise<{ list: CategoriesSelect[]; total: number }> => {
 ...
 * @returns
 */
export const getDbArticleByCategoriesId = async (
  id: number,
  page: number,
  pageSize: number,
 ...
 * @returns
 */
export const getOrAddCategoryByName = async (name: string): Promise<CategoriesSelect> => {
  // 先查询
...
export const deleteDbNotExistArticleCategory = async (name: string, tx?: DrizzleTx) => {
